...
**Correzione importante**: il display del Watcher è **412x412** (non 320x320 come scritto nel doc originale). Tutti i layout LVGL e asset devono usare 412.
...
function sanitizeForTts(text: string): string {
  return (
    text
      ...
      .replace(/[~>`]/g, "")
      // Emoji-like patterns (opzionale, se TTS li legge male)
...
- `.replace(/[*_]/g, "")` — cattura asterischi/underscore isolati che le regex precedenti non matchano
- `.replace(/```[\s\S]*?```/g, "")` — rimuove blocchi di codice interi
- `.replace(/^[-*_]{3,}\s*$/gm, "")` — rimuove linee orizzontali
...
**`extensions/xiaozhi/src/audio-pipeline.ts`** — 7 punti di inserimento:
...
**Obiettivo**: il firmware riceve `SET_UI` e renderizza i 7 stati di Ada sul display 412x412.
...
**Obiettivo**: asset grafici per i 7 stati, convertiti in C-array LVGL.
...
**Obiettivo**: spegnimento/deep sleep affidabile per i tester.
...
- **FreeRTOS**: `Task_Audio_BT` priorità 22, `Task_Comm_WiFi` priorità standard
